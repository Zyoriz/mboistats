...
class LoggerService {
  ...
  /// Sumber kebenaran: tabel `categories` di Supabase (lihat categories_rows.csv export).
  /// PENTING: id 2/3/4 BUKAN urutan alfabetis/logis — ikuti persis nilai live di database.
  ...
  static Future<void> init() async {
    ...
    try {
      // Cek apakah Supabase sudah diinisialisasi oleh main.dart
      final _ = Supabase.instance.client;
      _isInitialized = true;
      print('[LoggerService] Supabase client is already active.');
      return;
    } catch (_) {
      // Belum diinisialisasi, lakukan inisialisasi di bawah
    }
    ...
    try {
      ...
      print('[LoggerService] Status inisialisasi Supabase: $e');
      _isInitialized = true;
    }
  }
  ...
  /// Mendukung parameter skema baru DDL Supabase & kompatibel dengan seluruh pemanggilan legacy.
  static Future<void> logActivity({
    ...
    final effectiveModule = (moduleName != 'fitur')
        ? moduleName
        : (contentType != null && contentType.isNotEmpty ? contentType : 'fitur');
    ...
    print('[Activity Log] [$platformName] User: $activeUserId | Action: $actionType | CategoryID: $effectiveCategoryId ($effectiveSector) | Title: "$effectiveTitle" | Module: $effectiveModule');
